    ...
    <section className="section-apple relative">
      ...
      <div className="relative mx-auto max-w-[1000px] px-6 lg:px-8">
        <Reveal className="text-center">
          ...
          <h2 className="section-title font-apple text-white mt-3">See how you <span className="gradient-text">save.</span></h2>
        </Reveal>

        <Reveal delay={100} className="mt-14 overflow-hidden rounded-[24px] border border-white/[0.06]">
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody>
                {ROWS.map((r, i) => (
                  <tr key={r.repair} className={`border-b border-white/[0.04] transition-colors hover:bg-white/[0.02] ${i === ROWS.length - 1 ? "border-b-0" : ""}`}>
                    <td className="px-6 py-4 text-[14px] font-medium text-white">{r.repair}</td>
        ...
        <Reveal delay={200} className="mt-8 text-center">
          <a href={wa("Hi! I want the best price for my repair. 🍎")} target="_blank" rel="noopener noreferrer"
            className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-[#ff2d55] to-[#ff9500] px-7 py-3.5 text-[14px] font-bold text-white transition-all hover:scale-[1.03] hover:shadow-lg hover:shadow-[#ff2d55]/20">
